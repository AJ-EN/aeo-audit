...
### Fixed
- **Zero-setup first run.** `pip install aeo-audit` doesn't pull the Chromium
  binary Playwright needs, so a fresh machine's first scan used to fail with a
  raw Playwright error. The crawler now detects the missing browser, installs it
  automatically (one-time, ~30s), and retries — with a clear fallback message if
  that fails. `pipx install aeo-audit && aeo-audit scan <url>` now just works.
...
### Fixed
- **Percentile grading now works for installed users.** The benchmark corpus
  shipped at the repo root, outside the wheel, so `pip`-installed runs silently
  fell back to absolute grading (every site scored F). The corpus now lives in
  `aeo_audit/benchmarks/` and ships with the package.
- **`--version` now reports the real version.** It is derived from the installed
  distribution metadata instead of a hardcoded string that had drifted.
